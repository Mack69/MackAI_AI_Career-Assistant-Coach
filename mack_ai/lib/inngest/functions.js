...
export const generateIndustryInsights = inngest.createFunction(
  {
    id: "generate-industry-insights", // Required: unique ID
    name: "Generate Industry Insights", // Optional: display name
    cron: "0 0 * * 0", // Run every Sunday at midnight
  },
  async ({ event, step }) => {
      ...
      await step.run(`Update ${industry} insights`, async () => {
        return await db.IndustryInsights.update({
          ...
          data: {
            ...
            demandLevel:
              result.demandLevel?.toUpperCase() === "HIGH"
                ? "HIGH"
                : result.demandLevel?.toUpperCase() === "LOW"
                  ? "LOW"
                  : "MEDIUM",
            topSkills: result.topSkills || [],
            marketOutlook:
              result.marketOutlook?.toUpperCase() === "POSITIVE"
                ? "POSITIVE"
                : result.marketOutlook?.toUpperCase() === "NEGATIVE"
                  ? "NEGATIVE"
                  : "NEUTRAL",
    ...
    return { success: true, message: "Industry insights updated" };
  },
);
